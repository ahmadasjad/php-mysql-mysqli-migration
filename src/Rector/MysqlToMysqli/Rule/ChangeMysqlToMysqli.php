...
class ChangeMysqlToMysqli extends AbstractRector
{
    ...
    private function refactorFunctionWithoutClass(Node\Expr\FuncCall $node, $functionCallName)
    {
        ...
        // Change params
        $node = $this->changeParams($node, $functionCallName);

        return $node;
    }

    private function changeParams(Node\Expr\FuncCall $node, $functionCallName)
    {
        if (array_key_exists($functionCallName, $this->optionalConParamRequiredConParamMap))
        {
            ...
            if (!isset($passedArgs[$this->optionalConParamRequiredConParamMap[$functionCallName]])) {
                $contArg = new Arg(new ConstFetch(new Name(self::getDbConnectionVariable())));
